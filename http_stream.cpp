...
void
http_stream::input(const char *buf, int len)
{
	int i;
	int j;
	int hdr_end = 0;
		...
		if (!m_ContentLength) {
			if (i > strlen(CONTENT_LEN)) {
				for (j = 0; j < i - strlen(CONTENT_LEN); j++) {
					if (!strncasecmp(buf + j, CONTENT_LEN,
							strlen(CONTENT_LEN))) {
						m_ContentLength = atoi(buf + j +
							strlen(CONTENT_LEN) +
							+ 2);
						break;
					}
...
int
http_stream::is_stream_end(void)
{
